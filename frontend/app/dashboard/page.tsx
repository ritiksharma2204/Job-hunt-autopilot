      ...
      <header className="mx-auto mb-10 flex max-w-5xl items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-amber-500/40 bg-amber-500/10">
            <span className="font-mono text-xs font-semibold text-amber-400">JA</span>
          </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="rounded-md border border-ink-700 px-3 py-1.5 text-xs font-medium text-fog-300 hover:border-signal-coral hover:text-signal-coral"
          >
            Log out
          </button>
        </div>
      ...
      <main className="mx-auto max-w-5xl space-y-8">
        <section className="rounded-xl border border-ink-700 bg-ink-900/60 p-5">
          <div className="mb-3 flex items-center justify-between">
            ...
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-md bg-amber-500 px-4 py-2 text-xs font-semibold text-ink-950 hover:bg-amber-400">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-lg border p-3 ${
                      isActive ? "border-amber-500/50 bg-amber-500/5" : "border-ink-700 bg-ink-950/50"
                    }`}
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-2">
                      {isActive ? (
                        <span className="rounded-full border border-amber-500/40 bg-amber-500/15 px-2.5 py-1 text-xs font-medium text-amber-400">
                          Active for matching
                        </span>
                      ) : (
                        <button
                          onClick={() => handleActivateResume(r.id)}
                          className="rounded-md border border-ink-700 px-3 py-1.5 text-xs font-medium text-fog-300 hover:border-amber-500/50 hover:text-amber-400"
                        >
                          Use for matching
                        </button>
                      )}
        ...
        <section className="rounded-xl border border-ink-700 bg-ink-900/60 p-5">
          ...
          <form onSubmit={handleSearch} className="grid grid-cols-1 gap-3 sm:grid-cols-4">
            ...
            <button
              type="submit"
              disabled={searching}
              className="rounded-md bg-signal-slate px-4 py-2 text-sm font-semibold text-fog-100 hover:opacity-90 disabled:opacity-50 sm:col-span-4"
            >
          ...
          <div className="mt-5 space-y-3">
            {jobs.map((job) => {
              ...
              return (
                <div
                  key={job.id}
                  className="flex flex-col gap-4 rounded-lg border border-ink-700 bg-ink-950/50 p-4 sm:flex-row sm:items-center"
                >
                  ...
                  <div className="flex shrink-0 flex-col gap-2 sm:items-end">
                    <div className="flex gap-2">
                      ...
                      <a
                        ...
                        className="rounded-md border border-signal-slate/50 px-3 py-1.5 text-xs font-medium text-signal-slate hover:bg-signal-slate/10"
                      >
                        Apply
                      </a>
                    </div>
                    <button
                      ...
                      className="rounded-md border border-ink-700 px-3 py-1.5 text-xs font-medium text-fog-300 hover:border-signal-teal hover:text-signal-teal disabled:opacity-50"
                    >
          ...
          {applications.length === 0 ? (
            ...
            <div className="space-y-3">
              {applications.map((app) => (
                <div
                  key={app.id}
                  className="flex flex-col gap-3 rounded-lg border border-ink-700 bg-ink-950/50 p-4 sm:flex-row sm:items-center sm:justify-between"
                >
                  <div>
                    <p className="font-display text-sm font-semibold text-fog-100">
                      {app.jobs?.title}
                    </p>
                    <p className="text-xs text-fog-300">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <select
                      ...
                      className="rounded-md border border-ink-700 bg-ink-950 px-2 py-1 text-xs text-fog-100 outline-none focus:border-amber-500"
                    >
